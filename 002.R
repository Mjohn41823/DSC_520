...
person_df1 <- read.csv("C:/Users/Matt/Desktop/person.csv")
str(person_df1)
...
person_df2 <-read.csv("C:/Users/Matt/Desktop/person.csv", stringsAsFactors = FALSE)
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv("C:/Users/Matt/Desktop/scores.csv")
summary(scores_df)
...
my_data<-read_xls("G04ResultsDetail2004-11-02.xls")
...
voter_turnout_df1 <- read_xls("G04ResultsDetail2004-11-02.xls",skip = 1)
...
voter_turnout_df2 <-read_xls("G04ResultsDetail2004-11-02.xls",skip = 2,col_names =c("ard_precint","ballot_Cast","registered_voters","voter_turnout"))
...
db <- dbConnect(RSQLite::SQLite(), dbname = "example.db")
...
table_names <- dbListTables("example.db")
